...
function MP_mis_datos()
{
    ...
    if (!(isset($_REQUEST['partial']))) {
        //echo "Empieza cabecera";
         get_header();
         //echo "Acaba cabecera";
    }
    
    else if($_REQUEST['partial'] != "true"){
        get_header();
    }
    //get_header();
    ...
    switch ($_REQUEST['proceso']) {
        case "listarJson":
            ...
            $MP_pdo = new PDO("mysql:host=" . DB_HOST . ";dbname=" . DB_NAME, DB_USER, DB_PASSWORD); 
            $result= $MP_pdo->prepare("SELECT * FROM A_GrupoCliente000");
            $result->execute();
                ...
                foreach ($rows as $row) {
                    ...
                    foreach ($row as $key => $val) {
                        ...
                        if ($key=="foto_file" and $val!=null){
                            echo "<td> <img src='/Lab/P1/img/$val' width='100' height='100'>", "</td>";
                        }
                        else{
                        echo "<td>", $val, "</td>";
                        }
